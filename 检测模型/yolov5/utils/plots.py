...
def change_cv2_draw(img, strs, local, sizes, color):
    cv2img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    pilimg = Image.fromarray(cv2img)
    draw = ImageDraw.Draw(pilimg)
    font = ImageFont.truetype("SIMYOU.TTF",sizes, encoding="utf-8")
    text_size = draw.textsize(strs, font)
    txt_bbox = (local[0], local[1] + 23, local[0] + text_size[0], local[1] + 23 - text_size[1] - 3)
    draw.rectangle(txt_bbox, fill=(color[2], color[1], color[0]))
    draw.text(local, strs, (255, 255, 255), font=font)
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        # print("+++++++++++++++++++", label, c2)
        img = change_cv2_draw(img, label, (c1[0], c1[1] - 23), 20, color)
        return img
